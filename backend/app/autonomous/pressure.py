...
LENSES: list[dict] = [
    {
        ...
        "prompt": (
            ...
            "CRITICAL — 'NOT EMPTY' IS NOT A SURVIVAL, but crowded is not a kill "
            "either. This lens tests the gap's white-space premise. If the space "
            "is CROWDED with funded competitors, the 'white space' premise is "
            "false, so do NOT return survives — return weakens (the pitch "
            "oversold openness, but a crowded market with a real demand signal is "
            "normal and winnable). Reserve kills for the structural traps only: a "
            "regulatory wall, no real willingness to pay, an unreachable channel, "
            "a market that has TIPPED to a dominant winner, or value COMMODITIZED "
            "to free — not the mere presence of competitors. A survival is a "
            "genuinely open space that is also not a trap. Never award a survival "
            "on a finding your own argument describes as bad news for the gap."
        ),
    },
    {
        "key": "incumbent_countermove",
        "prompt": (
            "INCUMBENT COUNTER-MOVE. Test whether the market is STRUCTURALLY "
            "CLOSED to a new entrant — not merely whether a competitor exists.\n"
            "A COMPETITOR EXISTING IS NOT A KILL — not even a near-identical one. "
            "That is the default state of every good market: Google was the ~20th "
            "search engine, Slack launched into a crowded chat market, Cursor "
            "entered after Copilot. A funded, near-identical competitor is a "
            "verdict=weakens that DEMANDS a differentiation/execution answer — it "
            "is not death. The question is never 'does a similar thing exist' but "
            "'can a new entrant still win a meaningful share'.\n"
            "verdict=kills ONLY on structural lock-out, of which there are two "
            "kinds: (1) the market has TIPPED — a dominant incumbent holds a real "
            "network effect, standard, or scale moat a new entrant genuinely "
            "cannot overcome (you won't beat Stripe at generic payments or start a "
            "new general web search vs Google); or (2) the exact value is "
            "COMMODITIZED TO ZERO — bundled free into a platform the buyer already "
            "runs, so there's no wallet to open. Absent one of those, the verdict "
            "is at most weakens.\n"
            "Use web_search to find the real competitive picture (named players, "
            "funding, whether a winner has emerged) — a competitor with a "
            "marketing site and a design win leaves no repo or paper. But weigh it "
            "correctly: contested-but-winnable is where nearly every venture "
            "outcome lives, so a crowded market with a genuine differentiated "
            "wedge should survive or weaken. Reserve the kill for a market that is "
            "actually won or a value that is actually free."
        ),
    },
    {
        "key": "demand_mirage",
